...
class PredictionDataCleaner:
    ...
    def _overlap_data(self):
        ...
        self.player_num = self.df["player"].iloc[0]
        ...
        if len(self.df) >= N:
            ...
            result_df = result_df.drop(self.df.columns, axis=1)
        else:
            return False
    ...
    def _append_df_to_csv(self):
        out_csv = self.transformed_dir + "/predict_data.csv"

        self.df["player"] = self.player_num

        file_exists = os.path.isfile(out_csv)
        last_row = self.df.iloc[-1:]
        last_row.to_csv(out_csv, mode="a", header=not file_exists, index=False)
    ...
    def clean_and_append_to_main(self, csv):
        ...
        has_enough_data = self._overlap_data()
